...
# 判定ガイド
- ignore: 内容が完全に空 (挨拶・つなぎ語・絵文字や記号単独・テスト入力・「ありがとう」などの応答ワード・「忘れて」などのメタ指示) のみ。
  日記的な事実 (天気・体調・行った場所・行動) は些細でも episodic として保存すること。判断に迷ったら ignore せず create を選ぶ。
- preference: ユーザーの好み・スタイル・嫌悪 (継続的な選好)
- fact: 客観的な事実・属性・環境・スペック (現時点の状態)
- project: 進行中のタスク・目標・成果物。**進捗報告・完了報告・状態変化も project として扱う**
- constraint: 守るべき制約・ルール・締切・期限
- episodic: 一時的な出来事・体験・行動の記録 (些細でも保存)
